...
// Default qualifications with date ranges
...
// Render qualifications on any page
function renderQualifications(containerId, showControls = false) {
  const quals = loadQualifications();
...
function handleQualificationSubmit(event) {
  ...
  const levelTag = document.getElementById('edit-level').value;
  ...
  if (id) {
    ...
    if (index !== -1) quals[index] = newQual;
  } else {
    // Add new
    quals.push(newQual);
  }
